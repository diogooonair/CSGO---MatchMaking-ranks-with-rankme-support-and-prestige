...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_setprestige", Cmd_SetPrestige, ADMFLAG_ROOT);
	...
	RegConsoleCmd("say", Settag);
	RegConsoleCmd("say_team", Settagteam);
...
public Action Cmd_SetPrestige(int client, int args)
{
	if(args < 6)
	{
		CReplyToCommand(client, "%t", "CNeedsToBe");
	...
	if(StringToInt(level) < 1 || StringToInt(level) > 5)
	{
		CReplyToCommand(client, "%t", "NeedsToBe");
		return Plugin_Handled;
    }
	...
	GetCmdArg(4, s4, 32);
	GetCmdArg(5, s5, 32);
	Format(s1, 32, "%s%s%s%s%s", s1, s2, s3, s4, s5);
	
	char g_Query[512];
	FormatEx(g_Query, 512, "UPDATE prestige_playerdb SET level = '%i' WHERE steamid = '%s'", StringToInt(level), s1);
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		...
		if(StrEqual(steamid, s1))
		{
			prestigelevel[i] = StringToInt(level);
	    }
	}
...
public Action Settagteam(int client, int args)
{
...
stock Action AddTag(int client, bool team)
{
	/* Get the name */
	char sName[64];
	...
	if(team)
	{
		...
		{
			Format(message, sizeof(message), "{teamcolor}[TEAM CHAT] {lightgreen}[PRESTIGE 1] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 2)
		{
			Format(message, sizeof(message), "{teamcolor}[TEAM CHAT] {lime}[PRESTIGE 2] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 3)
		{
			Format(message, sizeof(message), "{teamcolor}[TEAM CHAT] {green}[PRESTIGE 3] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 4)
		{
			Format(message, sizeof(message), "{teamcolor}[TEAM CHAT] {yellow}[PRESTIGE 4] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 5)
		{
			Format(message, sizeof(message), "{teamcolor}[TEAM CHAT] {gold}[PRESTIGE 5] {teamcolor}%s : {default}%s", sName, message);
		}	
		for(int i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i))
    ...
    else
    {
	    if(prestigelevel[client] == 1)
		{
			Format(message, sizeof(message), "{lightgreen}[PRESTIGE 1] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 2)
		{
			Format(message, sizeof(message), "{lime}[PRESTIGE 1] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 3)
		{
			Format(message, sizeof(message), "{green}[PRESTIGE 1] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 4)
		{
			Format(message, sizeof(message), "{yellow}[PRESTIGE 1] {teamcolor}%s : {default}%s", sName, message);
		}
		else if(prestigelevel[client] == 5)
		{
			Format(message, sizeof(message), "{gold}[PRESTIGE 1] {teamcolor}%s : {default}%s", sName, message);
		}
		...
		{
			CPrintToChatAllEx(client, "*DEAD* %s", message);
		}
		else if(!IsPlayerAlive(client) && GetClientTeam(client) != 2 && GetClientTeam(client) != 3)
		{
			CPrintToChatAllEx(client, "*SPEC* %s", message);
		}
    }
...
public Action Cmd_prestige(int client, int args)
{
	if(rank[client] == 18)
	{
	  ...
	  if(prestigelevel[client] == 1)
	  {
		...
		char steamid[512];
		GetClientAuthId(client, AuthId_Steam2, steamid, sizeof(steamid));
		FormatEx(g_Query, 512, "UPDATE prestige_playerdb SET level = '%i' WHERE steamid = '%s'", prestigelevel[client], steamid);
		SQL_TQuery(g_db, T_Generic, g_Query);
	  }
	  else if(prestigelevel[client] == 2)
	  {
...
public void OnClientPutInServer(int client)
{
	...
	FormatEx(gB_Query, 512, "SELECT model,tag,level FROM prestige_playerdb WHERE steamid='%s';", steamid);
...
public void SQL_CallBack2(Database db, DBResultSet results, const char[] error, any data)
{
	...
	if (!results.FetchRow())
	{
		...
		char steamid[512];
		GetClientAuthId(client, AuthId_Steam2, steamid, sizeof(steamid));
		FormatEx(gB_Query, 512, "INSERT INTO prestige_playerdb(steamid,model,tag, level) VALUES('%s','def','YES', '1');", steamid);
	...
	else
	{
		...
		prestigelevel[client] = results.FetchInt(2);
...
void ConnectToDatabase()
{
	...
	char g_Query[512];
	Format(g_Query, sizeof(g_Query), "CREATE TABLE IF NOT EXISTS `prestige_playerdb` (`steamid` VARCHAR(48) NOT NULL, `model` VARCHAR(128) NOT NULL, `tag` VARCHAR(12) NOT NULL, `level` INT(12) NOT NULL, PRIMARY KEY (`steamid`)) ");
